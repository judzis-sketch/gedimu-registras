...
Šios instrukcijos padės jums įdiegti savo `Next.js` programą į `Firebase App Hosting`. Tai leis jūsų programai veikti `Firebase` infrastruktūroje, užtikrinant sklandų visų dalių (tiek vartotojo sąsajos, tiek serverio logikos, pavyzdžiui, `Firebase Functions`) veikimą kartu.
...
3.  Proceso metu `Firebase CLI` užduos jums kelis klausimus:
    *   **Pasirinkite Firebase projektą:** Iš sąrašo pasirinkite tą patį `Firebase` projektą, kurį naudojote iki šiol (`studio-xxxxxxxx-xxxxx`).
